	...
	public class P3dPaintableTexture : P3dLinkedBehaviour<P3dPaintableTexture>
	{
		...
		public void ClearStates()
		{
			if (states != null)
			{
				...
				{
					RenderTexture.ReleaseTemporary(states[i]);
				}
		...
		public void Undo()
		{
			if (stateIndex > 0)
			{
				...
				P3dPainter.Replace.Blit(current, state);
			}
		}
		...
		{
			var state = RenderTexture.GetTemporary(current.width, current.height, 0, current.format);

			P3dPainter.Replace.Blit(state, current);
		...
		public void Save(string saveName)
		{
			if (activated == true)
			{
				P3dHelper.SaveTexture(current, saveName);
			}
		}
		...
		{
			P3dHelper.LoadTexture(current, saveName);
		}
		...
		public void Activate()
		{
			if (activated == false)
			{
				...
				if (material != null)
				{
					...
					current = RenderTexture.GetTemporary(finalWidth, finalHeight, 0, format);
					//current = new RenderTexture(finalWidth, finalHeight, 0, format);
#if UNITY_EDITOR
		...
		public void ExecuteCommands(P3dPaintable paintable)
		{
			if (activated == true)
			{
				...
				if (previewSet == true)
				{
					RenderTexture.ReleaseTemporary(preview); previewSet = false;
				}
				...
				if (commandCount > 0)
				{
					...
					for (var i = 0; i < commandCount; i++)
					{
						...
						if (command.Preview != previewSet)
						{
							...
							{
								preview = RenderTexture.GetTemporary(current.width, current.height, current.depth, current.format); previewSet = true;

								P3dPainter.Replace.Blit(preview, current);
						...
						if (command.RequireSwap == true)
						{
							if (swapSet == false)
							{
								swap = RenderTexture.GetTemporary(current.width, current.height, current.depth, current.format); swapSet = true;
							}
						...
						if (command.Execute(swap, channel) == P3dCommand.RenderAs.Mesh)
						{
							if (prepared == false)
							{
								prepared = true;
				...
				if (swapSet == true)
				{
					RenderTexture.ReleaseTemporary(swap); swapSet = false;
				}
