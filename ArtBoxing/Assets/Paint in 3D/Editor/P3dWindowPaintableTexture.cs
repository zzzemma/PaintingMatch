...
namespace PaintIn3D
{
	public class P3dWindowPaintableTexture
	{
		...
		{
			var snapshot = new RenderTexture(source.width, source.height, 0);

			P3dPainter.Replace.Blit(snapshot, source);
		...
		public void Revert()
		{
			if (NewTexture != null)
			{
				...
				P3dPainter.Replace.Blit(NewTexture, OldTexture);
			}
		}
		...
		public RenderTexture PreparePaint()
		{
			if (Locked == true)
			{
				...
				if (NewTexture == null)
				{
					NewTexture = new RenderTexture(OldTexture.width, OldTexture.height, 0, RenderTextureFormat.ARGB32);
